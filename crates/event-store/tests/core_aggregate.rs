...
fn core_transactions_are_atomic_revision_checked_and_idempotent() {
    ...
    let first = CoreTransaction {
        ...
        events: vec![CoreEvent {
            ...
            event_type: "child.created".into(),
            payload: json!({"contextId":"child-1"}),
        }],
    ...
    assert_eq!(
        store
            .load_core_aggregate(&first.aggregate_id)
            .unwrap()
            .unwrap()
            .revision,
        1
    ...
    assert_eq!(
        store.core_events_after(&first.aggregate_id, 0, 10).unwrap()[0]
            .1
            .event_id,
        "event-1"
    );

    let stale = CoreTransaction {
        transaction_id: "transaction-2".into(),
        events: vec![CoreEvent {
            event_id: "event-2".into(),
            event_type: "child.running".into(),
            payload: json!({}),
        }],
        ..first
    };
    assert!(matches!(
        store.append_core_transaction(&stale),
        Err(StoreError::CoreRevisionConflict {
            expected: 0,
            actual: 1
        })
    ));
...
fn core_transactions_reject_empty_event_batches() {
    ...
    assert!(matches!(
        store.append_core_transaction(&invalid),
        Err(StoreError::InvalidCoreTransaction(_))
    ));
}
