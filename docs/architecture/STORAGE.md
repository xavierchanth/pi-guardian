...
A private, domain-neutral artifact substrate stores bounded opaque blobs under the centralized XDG data root, partitioned by normalized root-session identifiers. Records are immutable, digest verified, mode-hardened before durable atomic publication, and identical publication is retry-safe. Private metadata retains the opaque lookup key for recovery; bounded cleanup removes abandoned staging records. It is not a report or image store, and runtime storage is never durable authority.
