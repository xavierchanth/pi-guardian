...
| **Orchestrator** | Root agent that grounds requests, collaborates with the user on design and planning, records approval, routes approved work, and owns review disposition, integration, and final verification. |
| **Implementation Lead** | Agent owning one approved product task in a dedicated workspace, including direct implementation or decomposition into bounded Worker assignments. |
| **Documenter** | Non-delegating agent materializing one approved standalone architecture, design, documentation, or roadmap update within explicit documentation paths. |
...
| **task tree** | Durable state-owned intent hierarchy: immutable Orchestrator goal, sourced user directions, child assignments, effective plans, and explicit user-evidenced plan approvals backed by append-only records; execution roles receive effective-only projections while Reviewers receive immutable full-history snapshots. |
