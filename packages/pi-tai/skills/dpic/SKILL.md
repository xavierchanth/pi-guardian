---
name: dpic
description: Use when work is large enough to have parts and should be designed, planned, implemented through subagents, and closed out — multi-file features, refactors, migrations, or any request where implementation should be delegated to subagents while design and review stay in the main conversation.
---
...
Start it like any other subagent, with `model: "opus"` and `isolation: "shared"` so it reads the working copy as it actually stands, including uncommitted work. Use `fable` only if the user names it. Its opening objective should carry the problem, what has already been established, what has been ruled out and why, and the specific judgment being asked for — it sees none of this conversation, and a partner briefed as though it does will answer a question nobody asked.
...
Each exchange is a turn: send a message, the partner runs, its reply comes back. This is the one place `subagent_wait` is genuinely the right call — a dialogue cannot continue without the answer, so there is nothing useful to do in the meantime. That is the opposite of the implementation phase, where waiting is a mistake.
...
Implementation runs on `sol` by default, which suits most pieces. Use `fable` only when the user asks for it by name.
