...
import type {
  RepositoryEnrollmentReceiptV1,
  RepositoryMutationLeaseV1,
  SessionWorkspaceCustodyV1,
  SessionWorkspaceIdentityV1,
...
export interface WorkspaceHostServicePort {
...
export class HostSessionWorkspaceStore implements SessionWorkspaceStore {
  ...
  constructor(host: WorkspaceHostServicePort, rootSessionId: string) {
    this.host = host;
  ...
  async get(workspaceId: string): Promise<SessionWorkspaceCustodyV1 | undefined> {
    ...
    if (!aggregate) {
      this.revisions.set(workspaceId, 0);
      return undefined;
    }
    if (
      ...
      throw new Error("Host session workspace aggregate is invalid.");
  ...
  async put(workspaceId: string, custody: SessionWorkspaceCustodyV1): Promise<void> {
    if (
      ("rootSessionId" in custody ? custody.rootSessionId : custody.identity.rootSessionId) !==
      this.rootSessionId
    )
      throw new Error("Session workspace custody belongs to another Host session.");
    const expectedRevision =
      this.revisions.get(workspaceId) ??
      (await this.get(workspaceId), this.revisions.get(workspaceId) ?? 0);
    const aggregate = await this.host.request<any>("session.workspace.put", {
      workspaceId,
      transactionId: `session-workspace-transaction-${randomUUID()}`,
      expectedRevision,
      value: custody,
    });
    if (!Number.isSafeInteger(aggregate?.revision) || aggregate.revision !== expectedRevision + 1)
      throw new Error("Host did not durably advance session workspace custody.");
    this.revisions.set(workspaceId, aggregate.revision);
  }
  async list(): Promise<SessionWorkspaceCustodyV1[]> {
    throw new Error("Host session workspace listing uses bounded concurrency projections.");
  }
}
...
export class FileSessionWorkspaceStore implements SessionWorkspaceStore {
  ...
  async get(workspaceId: string): Promise<SessionWorkspaceCustodyV1 | undefined> {
    try {
      return JSON.parse(
        await readFile(this.path(workspaceId), "utf8"),
      ) as SessionWorkspaceCustodyV1;
    } catch (error) {
  ...
  async list(): Promise<SessionWorkspaceCustodyV1[]> {
    ...
    try {
      const output: SessionWorkspaceCustodyV1[] = [];
      for (const name of (await readdir(this.root))
        .filter((item) => item.endsWith(".json"))
        .sort()) {
        const value = await this.get(name.slice(0, -5));
...
export class InMemoryRepositoryLeaseStore implements RepositoryLeaseStore {
  ...
  get(repositoryId: string): Promise<RepositoryMutationLeaseV1 | undefined> {
    return Promise.resolve(this.states.get(repositoryId));
  }
  put(repositoryId: string, lease: RepositoryMutationLeaseV1): Promise<void> {
    this.states.set(repositoryId, lease);
...
export interface RepositoryMutationCoordinatorPort {
  withLease<T>(
    ...
    action: (lease: Extract<RepositoryMutationLeaseV1, { phase: "leased" }>) => Promise<T>,
  ): Promise<T>;
}
...
export class RepositoryMutationCoordinator implements RepositoryMutationCoordinatorPort {
  ...
  constructor(store: RepositoryLeaseStore, now: () => string = () => new Date().toISOString()) {
    this.store = store;
    this.now = now;
  }
  ...
  async withLease<T>(
    ...
    this.queues.set(input.repositoryId, chain);
    await prior;
    try {
      const current = await this.store.get(input.repositoryId);
      if (current?.phase === "leased" || current?.phase === "interrupted")
        throw new Error(
          `Repository ${input.repositoryId} has unresolved mutation authority in phase ${current.phase}.`,
        );
      ...
      try {
        ...
        await this.store.put(input.repositoryId, {
          ...
          repositoryId: input.repositoryId,
          generation,
          priorLeaseId: lease.leaseId,
          priorRootSessionId: lease.rootSessionId,
          priorRuntimeGeneration: lease.runtimeGeneration,
      ...
      release();
      if (this.queues.get(input.repositoryId) === chain) this.queues.delete(input.repositoryId);
    }
  }
  ...
  async reconcile(repositoryId: string, proof: { mutationDidNotStart: boolean }): Promise<void> {
    ...
    if (!proof.mutationDidNotStart)
      throw new Error(
        "Interrupted repository mutation requires semantic operation reconciliation before lease release.",
      );
    await this.store.put(repositoryId, {
      phase: "available",
      repositoryId,
...
export interface RepositoryHostServicePort {
  request<T = unknown>(method: string, params: unknown): Promise<T>;
}
...
export class HostRepositoryMutationCoordinator implements RepositoryMutationCoordinatorPort {
  ...
  constructor(host: RepositoryHostServicePort) {
    this.host = host;
  }
  async withLease<T>(
    input: {
    ...
    while (true) {
      try {
        lease = await this.host.request<Extract<RepositoryMutationLeaseV1, { phase: "leased" }>>(
        ...
        if (
          !/repository is busy/i.test(error instanceof Error ? error.message : String(error)) ||
          Date.now() >= deadline
        )
          throw error;
    ...
    if (
      lease.phase !== "leased" ||
      lease.repositoryId !== input.repositoryId ||
      lease.operationId !== input.operationId
    )
      throw new Error("Host returned invalid repository mutation authority.");
    try {
      const output = await action(lease);
      await this.host.request("repository.lease.release", {
        repositoryId: input.repositoryId,
        leaseId: lease.leaseId,
        operationId: input.operationId,
        transactionId: `repository-lease-release-${randomUUID()}`,
      });
      return output;
    } catch (error) {
      await this.host
        .request("repository.lease.interrupt", {
          repositoryId: input.repositoryId,
          leaseId: lease.leaseId,
          operationId: input.operationId,
          reason: error instanceof Error ? error.message : String(error),
          transactionId: `repository-lease-interrupt-${randomUUID()}`,
        })
        .catch(() => undefined);
      throw error;
    }
...
export class SessionWorkspaceService {
  ...
  constructor(options: {
    ...
    coordinator: RepositoryMutationCoordinatorPort;
    executor?: JjExecutor;
    now?: () => string;
  ...
  async allocate(input: {
    enrollment: RepositoryEnrollmentReceiptV1;
    invokingCwd: string;
    rootSessionId: string;
    runtimeGeneration: number;
  }): Promise<SessionWorkspaceIdentityV1> {
    validateId(input.rootSessionId);
    const workspaceId = `session-workspace-${input.rootSessionId}`
      .replace(/[^a-zA-Z0-9_-]/g, "-")
      .slice(0, 128);
    const workspaceName = `pi-tai-${shortId(input.rootSessionId)}`;
    const path = join(
      input.enrollment.managedWorkspaceRoot,
      "sessions",
      shortId(input.rootSessionId),
    );
    ...
    if (existing?.phase === "ready") {
      ...
      if (
        identity.repositoryId !== input.enrollment.repositoryId ||
        identity.rootSessionId !== input.rootSessionId ||
        resolve(identity.path) !== resolve(path)
      )
        throw new Error(
          "Persisted session workspace custody conflicts with repository or session identity.",
        );
      const orchestrationChangeId = line(
        await this.run(
          identity.path,
          ["log", "--revision", "@", "--no-graph", "--template", CHANGE_ID_TEMPLATE],
          "read",
        ),
        "recovered session orchestration Change ID",
      );
      if (orchestrationChangeId !== identity.orchestrationChangeId)
        throw new Error(
          "Persisted session workspace no longer matches its acknowledged custody identity.",
        );
    ...
    if (existing) {
      if (existing.phase === "allocating")
        await this.store.put(workspaceId, {
          ...
          rootSessionId: input.rootSessionId,
          workspaceId,
          lastSafeBoundary: "prepared",
          reason:
            "Host restarted during session workspace allocation; explicit repair is required before any JJ mutation.",
          evidence: existing,
          stoppedAt: this.now(),
        });
      throw new Error(
        `Session workspace custody is ${existing.phase}; explicit recovery or cleanup is required.`,
      );
    }
    ...
    try {
      return await this.coordinator.withLease(
        {
          repositoryId: input.enrollment.repositoryId,
          rootSessionId: input.rootSessionId,
          runtimeGeneration: input.runtimeGeneration,
        ...
        async () => {
          ...
          const sourceWorkspaceName = currentWorkspace(
            await this.run(
              input.invokingCwd,
          ...
          await this.run(
            ...
            [
              ...
              workspaceName,
              "--revision",
              exactChange(baseChangeId),
              "--message",
              `pi-tai: session ${input.rootSessionId}`,
            ],
            "write",
          );
          ...
          const orchestrationChangeId = line(
            ...
            "session orchestration Change ID",
          );
      ...
      await this.store.put(workspaceId, {
        ...
        rootSessionId: input.rootSessionId,
        workspaceId,
        lastSafeBoundary: current?.phase === "allocating" ? "prepared" : "workspace_added",
        reason: error instanceof Error ? error.message : String(error),
        evidence: current,
        stoppedAt: this.now(),
      });
  ...
  async retryCleanup(input: {
    ...
    if (current.phase !== "ready" && current.phase !== "cleanup_pending")
      throw new Error(`Session workspace cleanup cannot start from ${current.phase}.`);
    const identity = current.identity;
    if (identity.repositoryId !== input.enrollment.repositoryId)
      throw new Error("Session workspace cleanup crossed repository enrollment authority.");
    await this.store.put(workspaceId, {
      version: 1,
    ...
    try {
      await this.coordinator.withLease(
        {
          repositoryId: identity.repositoryId,
          rootSessionId: input.rootSessionId,
          runtimeGeneration: input.runtimeGeneration,
          operationId: `session-cleanup-${randomUUID()}`,
        },
        async () => {
          const patch = await this.run(identity.path, ["diff", "--revision", "@", "--git"], "read");
          if (patch.trim())
            throw new Error(
              "Session orchestration change still contains unintegrated work; cleanup refuses to discard it.",
            );
          await this.run(
            input.enrollment.canonicalRepositoryPath,
            ["workspace", "forget", identity.workspaceName],
            "write",
          );
      ...
      await this.store.put(workspaceId, {
        ...
        workspaceId,
        identity,
        lastSafeBoundary: "cleanup_pending",
        reason: error instanceof Error ? error.message : String(error),
        evidence: current,
        stoppedAt: this.now(),
      });
...
function currentWorkspace(output: string, currentChangeId: string): string {
  const names = output
    ...
    .filter(Boolean)
    .map((row) => row.split("|", 2))
    .filter(([, id]) => id === currentChangeId)
    .map(([name]) => name!);
  if (names.length !== 1) throw new Error("Unable to resolve exactly one invoking JJ workspace.");
  return names[0]!;
}
function exactChange(value: string): string {
  if (!/^[a-z]{32}$/.test(value)) throw new Error(`Invalid full JJ Change ID: ${value}`);
  return `exactly(change_id(${value}), 1)`;
}
function line(output: string, label: string): string {
  const values = output
    ...
    .filter(Boolean);
  if (values.length !== 1) throw new Error(`Unable to resolve one ${label}.`);
  return values[0]!;
}
...
function requireInside(root: string, path: string): void {
  ...
  if (target !== base && !target.startsWith(`${base}${sep}`))
    throw new Error("Managed session workspace escaped its enrolled root.");
  if (relative(base, target).split(sep).includes(".."))
    throw new Error("Managed session workspace escaped its enrolled root.");
}
function validateId(value: string): void {
  if (!/^[a-zA-Z0-9][a-zA-Z0-9_.:-]{0,127}$/.test(value))
    throw new Error(`Invalid managed ID: ${value}`);
}
