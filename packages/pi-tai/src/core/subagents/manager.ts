...
interface Entry {
  ...
  closed: boolean;
  /** Historical records have no live process, but terminal Claude/Codex records may reopen. */
  restored: boolean;
  /** Folded authority retained privately for explicit reload continuation gates. */
  reloadRecord?: import("./lifecycle.ts").LifecycleRecord;
  /** Workspace custody is independent of the historical workspace identifier. */
  custodyResolved: boolean;
  /** Exact durable evidence proves the historical workspace can never be reopened. */
  custodyRetired: boolean;
...
export class SubagentManager {
  ...
  async attachLifecycleStore(store: SubagentLifecycleStore): Promise<void> {
    ...
    for (const record of folded.records.values()) {
      ...
      // A running Codex process is especially conservative: without an exported,
      // matching OS identity we cannot prove it dead. Mark it as an orphaned
      // custody incident so cancel, send, pruning, and workspace sweep all fail closed.
      const orphaned = record.disposition === "running";
      ...
      this.entries.set(record.displayId, {
        ...
        restored: true,
        reloadRecord: record,
        // A workspace identifier is evidence, never proof that custody vanished.
        // The isolation adapter must restore or explicitly resolve ownership.
        custodyResolved: record.workspaceId === undefined,
        custodyRetired: false,
      });
    }
    this.generation += 1;
  }
  ...
  async spawn(request: SpawnRequest): Promise<SubagentSnapshot> {
    ...
    try {
      ...
      try {
        ...
        // A persisted running fact must always be followed by a durable terminal
        // downgrade before spawn failure is returned to the caller.
  ...
  private async dispatchSend(
    ...
    if (entry.restored) {
      if (requested !== "continue")
        throw new Error(
          `Subagent ${id} was restored after reload; only explicit mode:'continue' may reopen it.`,
        );
      await this.continueReloaded(entry, text);
  ...
  private async continueReloaded(entry: Entry, text: string): Promise<void> {
    const record = entry.reloadRecord;
    // All purely local gates precede availability and spawn. In particular an
    // orphan is never probed through, mutated, interrupted, or continued.
    if (!record || entry.snapshot.attention || record.disposition === "running")
      throw new Error(`Subagent ${entry.snapshot.id} has unproved live ownership after reload.`);
    if (!["done", "failed", "interrupted", "cancelled"].includes(record.disposition))
      throw new Error(`Subagent ${entry.snapshot.id} is not terminal.`);
    if (record.backend !== "claude" && record.backend !== "codex")
      throw new Error(`The ${record.backend} backend cannot be reopened after reload.`);
    if (record.rootSessionId !== this.rootSessionId)
      throw new Error(`Subagent ${entry.snapshot.id} belongs to a different root session.`);
    const expectedKind = record.backend === "claude" ? "claude_session" : "codex_thread";
    if (!record.resumeHandle || record.resumeHandle.kind !== expectedKind)
      throw new Error(`Subagent ${entry.snapshot.id} has no expected continuation handle.`);
    if (!record.cwd || record.cwd !== entry.snapshot.cwd || entry.custodyResolved === false)
      throw new Error(`Subagent ${entry.snapshot.id} cannot prove workspace cwd/custody.`);
    if (entry.custodyRetired)
      throw new Error(`Subagent ${entry.snapshot.id} has retired workspace custody.`);
    let cwd;
    try {
      cwd = await stat(record.cwd);
    } catch {
      throw new Error(
        `Subagent ${entry.snapshot.id} cannot prove its retained workspace directory.`,
      );
    }
    if (!cwd.isDirectory())
      throw new Error(
    ...
    const config = record.backendConfig ?? {};
    const backend = this.registry.get(record.backend);
    if (!backend) throw new Error(`Backend "${record.backend}" is not enabled.`);
    if (config.model && !backend.capabilities.modelSelection)
      throw new Error(`Backend ${record.backend} cannot honor the retained model configuration.`);
    if (config.effort && !backend.capabilities.reasoningEffort)
      throw new Error(`Backend ${record.backend} cannot honor the retained reasoning effort.`);
    // Retained tool policy is authority, not a hint. Current external harnesses
    // do not expose a capability for reconstructing a restricted tool set.
    if (config.tools?.length)
      throw new Error(`Backend ${record.backend} cannot prove the retained tool configuration.`);
    if (!this.lifecycleStore && this.requireLifecycleStore)
      throw new Error("Subagent lifecycle persistence is unavailable; refusing continuation.");
    if (this.reserved >= this.maxRunning) this.assertAdmission();
    const availability = await backend.available();
    if (!availability.ok)
      throw new Error(`Backend "${record.backend}" is unavailable: ${availability.reason}`);
    ...
    try {
      ...
      const task: SpawnTask = {
        ...
        durableId: record.durableId,
        prompt: text,
        systemPrompt: record.charter ?? "Continue the retained subagent conversation.",
        cwd: record.cwd,
        title: record.title,
        resumeToken: record.resumeHandle.value,
      ...
      Object.assign(entry, { task, session, settled, resolveSettled, runToken, restored: false });
      ...
      // Once running was persisted, backend failure is itself a terminal run.
      // Keep the restored entry retryable, but make the folded lifecycle truthful.
      if (entry.generation === nextGeneration && !entry.generationAwaitingRunning) {
        await this.appendLifecycle({
          version: 2,
          type: "terminal",
          durableId: record.durableId,
  ...
  /** Private custody claims requiring proof by the isolation authority. */
  unresolvedCustody(): readonly {
    id: string;
    durableId: string;
    ...
    return [...this.entries.values()].flatMap((entry) => {
      const workspaceId = entry.snapshot.workspaceId ?? entry.reloadRecord?.workspaceId;
      if (entry.custodyResolved || !workspaceId) return [];
      return [
        {
          id: entry.snapshot.id,
          durableId: entry.snapshot.durableId,
          rootSessionId: entry.reloadRecord?.rootSessionId ?? this.rootSessionId,
          workspaceId,
  ...
  resolveCustody(id: string, retired = false): void {
    const entry = this.entries.get(id);
    if (!entry) return;
    entry.custodyResolved = true;
    entry.custodyRetired = retired;
    // Custody evidence alone says nothing about a reload-orphaned process. No
    // persisted process identity exists today, so attention remains fail-closed.
    this.prune();
  }
