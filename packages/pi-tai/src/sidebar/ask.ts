...
const SYSTEM_PROMPT = "Answer a one-off side question about this in-progress coding session. Use the supplied session context, but do not claim to use tools: none are available. Your answer is shown only to the user and will not be seen by the coding agent.";
...
export async function askBtw(input: {
  ...
  if (available < 512) throw new BtwContextTooLargeError("Question and system prompt leave too little room for an answer.");
