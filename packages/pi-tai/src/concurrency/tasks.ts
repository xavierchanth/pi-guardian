...
export interface PersistedTaskV1 {
  ...
  readonly assignment?: { readonly objective: string; readonly acceptanceCriteria: readonly string[]; readonly constraints: readonly string[]; readonly planBinding?: TaskPlanBindingV1 };
...
export function validateTask(input: unknown): PersistedTaskV1 {
  ...
  if ((input.ownerRole === "implementation-lead" || input.ownerRole === "documenter") !== Boolean(input.assignment?.planBinding)) throw new Error("Implementation Lead and Documenter assignments must bind exactly one Orchestrator plan.");
