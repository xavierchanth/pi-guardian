...
export const WORK_ORDER_EXECUTION_CLASSES = ["small-product", "large-product", "documentation"] as const;
export type WorkOrderExecutionClass = (typeof WORK_ORDER_EXECUTION_CLASSES)[number];
export interface WorkOrderResourceV1 { readonly type: "file" | "directory" | "url" | "commit"; readonly value: string; readonly reason?: string; }
...
export interface TaskPlanBindingV1 { readonly planRevisionId: string; readonly planDigest: string; readonly directionCount: number; }
...
export interface PersistedTaskV1 {
  ...
  readonly assignment?: { readonly executionClass?: WorkOrderExecutionClass; readonly objective: string; readonly instructions?: string; readonly acceptanceCriteria: readonly string[]; readonly constraints: readonly string[]; readonly resources?: readonly WorkOrderResourceV1[]; readonly validationRequirements?: readonly string[]; readonly documentationRequirements?: readonly string[]; readonly statusUpdates?: readonly string[]; readonly planBinding?: TaskPlanBindingV1 };
...
export class TaskService {
  ...
  async assign(parentTaskId: string, input: { ownerRole: Exclude<TaskOwnerRole, "orchestrator">; creatorContextId: string; objective: string; instructions?: string; executionClass?: WorkOrderExecutionClass; acceptanceCriteria?: readonly string[]; constraints?: readonly string[]; resources?: readonly WorkOrderResourceV1[]; validationRequirements?: readonly string[]; documentationRequirements?: readonly string[]; statusUpdates?: readonly string[] }): Promise<PersistedTaskV1> { const parent = await this.require(parentTaskId); const at = this.now(); if (input.ownerRole === "worker") await this.requireImplementationPlan(parentTaskId); const planBinding = input.ownerRole === "implementation-lead" || input.ownerRole === "documenter" || input.ownerRole === "worker" ? await this.currentPlanBinding(parentTaskId) : undefined; const executionClass = input.executionClass ?? executionClassForRole(input.ownerRole); if (executionClass && ownerRoleForExecutionClass(executionClass) !== input.ownerRole) throw new Error(`Execution class ${executionClass} does not match owner role ${input.ownerRole}.`); const child: PersistedTaskV1 = { version: 1, taskId: `task-${randomUUID()}`, rootSessionId: parent.rootSessionId, parentTaskId, ownerRole: input.ownerRole, createdByContextId: input.creatorContextId, goal: parent.goal, assignment: { ...(executionClass ? { executionClass } : {}), objective: bounded(input.objective, "assignment objective", 16_000), ...(input.instructions ? { instructions: bounded(input.instructions, "assignment instructions", 64_000) } : {}), acceptanceCriteria: boundedList(input.acceptanceCriteria ?? [], "assignment acceptance criteria"), constraints: boundedList(input.constraints ?? [], "assignment constraints"), ...(input.resources?.length ? { resources: validateResources(input.resources) } : {}), ...(input.validationRequirements?.length ? { validationRequirements: boundedList(input.validationRequirements, "validation requirements") } : {}), ...(input.documentationRequirements?.length ? { documentationRequirements: boundedList(input.documentationRequirements, "documentation requirements") } : {}), ...(input.statusUpdates?.length ? { statusUpdates: boundedList(input.statusUpdates, "status updates") } : {}), ...(planBinding ? { planBinding } : {}) }, execution: { phase: "unassigned" }, childTaskIds: [], directions: [], planRevisions: [], createdAt: at, updatedAt: at }; await this.store.create(child); await this.store.update(parentTaskId, (record) => ({ ...record, childTaskIds: [...record.childTaskIds, child.taskId], updatedAt: at })); return child; }
  ...
  async currentPlanBinding(taskId: string): Promise<TaskPlanBindingV1> { const root = await this.rootOf(taskId); const revision = root.planRevisions.at(-1); if (!revision) throw new Error("Implementation requires a current Orchestrator plan."); const latestDirection = root.directions.at(-1); if (latestDirection && revision.authorityMessageId !== latestDirection.evidence.messageId) throw new Error("Implementation requires a plan revised for the latest user direction."); return { planRevisionId: revision.revisionId, planDigest: createHash("sha256").update(revision.markdown).digest("hex"), directionCount: root.directions.length }; }
  async requireAssignmentPlan(taskId: string): Promise<TaskPlanBindingV1> { const task = await this.require(taskId); const binding = task.assignment?.planBinding; if (!binding) throw new Error("Workspace task is not bound to an Orchestrator plan."); return binding; }
  async requireImplementationPlan(taskId: string): Promise<TaskPlanBindingV1 | undefined> { let task = await this.require(taskId); if (!["implementation-lead", "documenter", "worker"].includes(task.ownerRole)) return undefined; while (!task.assignment?.planBinding && task.parentTaskId) task = await this.require(task.parentTaskId); if (!task.assignment?.planBinding) throw new Error("Writable task has no Orchestrator plan binding."); return task.assignment.planBinding; }
...
export function validateTask(input: unknown): PersistedTaskV1 {
  ...
  if (object(input.assignment)) {
    bounded(nonempty(input.assignment.objective, "assignment objective"), "assignment objective", 16_000);
    if (input.assignment.instructions !== undefined) bounded(nonempty(input.assignment.instructions, "assignment instructions"), "assignment instructions", 64_000);
    if (input.assignment.executionClass === undefined) input.assignment.executionClass = executionClassForRole(input.ownerRole);
    if (input.assignment.executionClass !== undefined) { if (!WORK_ORDER_EXECUTION_CLASSES.includes(input.assignment.executionClass)) throw new Error("Invalid work-order execution class."); if (ownerRoleForExecutionClass(input.assignment.executionClass) !== input.ownerRole) throw new Error("Work-order execution class does not match its owner role."); }
    if (!Array.isArray(input.assignment.acceptanceCriteria) || !Array.isArray(input.assignment.constraints)) throw new Error("Task assignment lists are required.");
    boundedList(input.assignment.acceptanceCriteria, "assignment acceptance criteria");
    boundedList(input.assignment.constraints, "assignment constraints");
    if (input.assignment.resources !== undefined) validateResources(input.assignment.resources);
    for (const [key, label] of [["validationRequirements", "validation requirements"], ["documentationRequirements", "documentation requirements"], ["statusUpdates", "status updates"]] as const) if (input.assignment[key] !== undefined) boundedList(input.assignment[key], label);
  }
  ...
  if (input.assignment?.planBinding !== undefined) validatePlanBinding(input.assignment.planBinding);
  if ((input.ownerRole === "implementation-lead" || input.ownerRole === "documenter" || input.ownerRole === "worker") !== Boolean(input.assignment?.planBinding)) throw new Error("Writable work orders must bind exactly one Orchestrator instruction revision.");
...
function validatePlanBinding(value: unknown): void { if (!object(value)) throw new Error("Task plan binding is invalid."); managedId(nonempty(value.planRevisionId, "plan revision"), "plan revision"); if (!/^[a-f0-9]{64}$/.test(nonempty(value.planDigest, "plan digest"))) throw new Error("Invalid plan digest."); if (!Number.isSafeInteger(value.directionCount) || value.directionCount < 0) throw new Error("Invalid direction count."); }
...
function renderTaskSnapshot(tasks: readonly PersistedTaskV1[]): string { return tasks.map((task, index) => { const current = task.planRevisions.at(-1)?.revisionId; return [`${"#".repeat(Math.min(index + 1, 6))} Task ${task.taskId}`, `Owner: ${task.ownerRole}`, `Goal: ${task.goal.objective}`, ...(task.assignment ? [`Work order: ${task.assignment.objective}`, ...(task.assignment.executionClass ? [`Execution class: ${task.assignment.executionClass}`] : []), ...(task.assignment.instructions ? [`Instructions:\n\n${task.assignment.instructions}`] : []), ...(task.assignment.validationRequirements?.length ? [`Validation requirements:\n${task.assignment.validationRequirements.map((item) => `- ${item}`).join("\n")}`] : []), ...(task.assignment.documentationRequirements?.length ? [`Documentation requirements:\n${task.assignment.documentationRequirements.map((item) => `- ${item}`).join("\n")}`] : []), ...(task.assignment.statusUpdates?.length ? [`Status updates:\n${task.assignment.statusUpdates.map((item) => `- ${item}`).join("\n")}`] : [])] : []), ...task.directions.map((item) => `Direction ${item.directionId} (${item.evidence.messageId}): ${item.summary}\n\n${item.evidence.content}`), ...task.planRevisions.map((item) => `${item.revisionId === current ? "Current" : "Superseded"} plan revision ${item.revisionId} (${item.authorRole})${item.directionIds?.length ? `\nDirections: ${item.directionIds.join(", ")}` : ""}:\n\n${item.markdown}\n\nRationale: ${item.rationale}`), ...(task.assignment?.planBinding ? [`Implementation plan binding: ${task.assignment.planBinding.planRevisionId}\nDigest: ${task.assignment.planBinding.planDigest}\nDirections: ${task.assignment.planBinding.directionCount}`] : [])].join("\n\n"); }).join("\n\n"); }
...
export function ownerRoleForExecutionClass(value: WorkOrderExecutionClass): "worker" | "implementation-lead" | "documenter" { if (value === "small-product") return "worker"; if (value === "large-product") return "implementation-lead"; return "documenter"; }
function executionClassForRole(value: TaskOwnerRole): WorkOrderExecutionClass | undefined { if (value === "worker") return "small-product"; if (value === "implementation-lead") return "large-product"; if (value === "documenter") return "documentation"; return undefined; }
function validateResources(values: readonly WorkOrderResourceV1[]): WorkOrderResourceV1[] { if (!Array.isArray(values) || values.length > 64) throw new Error("work-order resources must contain at most 64 items."); return values.map((resource) => { if (!resource || !["file", "directory", "url", "commit"].includes(resource.type)) throw new Error("Invalid work-order resource."); return { type: resource.type, value: bounded(resource.value, "resource value", 8_000), ...(resource.reason ? { reason: bounded(resource.reason, "resource reason", 2_000) } : {}) }; }); }
