...
export class HostConcurrencyRepository {
  ...
  constructor(services: HostServiceClientPort) { this.services = services; }
  ...
  async transact(transaction: ConcurrencyTransactionV1): Promise<CoreAggregateSnapshotV1> {
    ...
    if (aggregate.revision !== transaction.expectedRevision + 1) throw new Error("Host committed an unexpected concurrency revision.");
    return aggregate;
  }
}

function parseAggregate(value: unknown): CoreAggregateSnapshotV1 {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Host core aggregate is invalid.");
  const input = value as Record<string, unknown>;
  if (typeof input.aggregateId !== "string" || typeof input.updatedAt !== "string" || !Number.isSafeInteger(input.revision) || !Number.isSafeInteger(input.runtimeGeneration)) {
  ...
  if (projection.revision !== input.revision) throw new Error("Host aggregate and concurrency projection revisions differ.");
  return { aggregateId: input.aggregateId, revision: input.revision as number, runtimeGeneration: input.runtimeGeneration as number, state: input.state, projection, updatedAt: input.updatedAt };
}
