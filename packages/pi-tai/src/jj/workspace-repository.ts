...
export class JjWorkspaceRepositoryKernel {
  ...
  private async identity(workspaceId: WorkspaceId): Promise<PersistedWorkspaceIdentityV1> { const record = await this.options.workspaces.get(workspaceId); if (!record || (record.phase !== "active" && record.phase !== "reported")) throw new Error(`Workspace ${workspaceId} has no tracked operational identity.`); return record.identity; }
