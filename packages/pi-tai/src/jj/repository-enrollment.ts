...
export interface EnrollmentHostServicePort { request<T = unknown>(method: string, params: unknown): Promise<T>; }
...
export class HostRepositoryEnrollmentStore implements RepositoryEnrollmentStore {
  ...
  constructor(host: EnrollmentHostServicePort) { this.host = host; }
  async get(key: string): Promise<RepositoryEnrollmentV1 | undefined> {
    ...
    if (!aggregate) { this.revisions.set(key, 0); return undefined; }
    if (!Number.isSafeInteger(aggregate.revision) || aggregate.revision < 1 || !aggregate.projection) throw new Error("Host repository enrollment aggregate is invalid.");
  ...
  async put(key: string, value: RepositoryEnrollmentV1): Promise<void> {
    const expectedRevision = this.revisions.get(key) ?? (await this.get(key), this.revisions.get(key) ?? 0);
    const aggregate = await this.host.request<any>("repository.enrollment.put", { key, transactionId: `enrollment-transaction-${randomUUID()}`, expectedRevision, value });
    if (!Number.isSafeInteger(aggregate?.revision) || aggregate.revision !== expectedRevision + 1) throw new Error("Host did not durably advance repository enrollment state.");
    this.revisions.set(key, aggregate.revision);
  }
}
...
export class FileRepositoryEnrollmentStore implements RepositoryEnrollmentStore {
  ...
  async get(key: string): Promise<RepositoryEnrollmentV1 | undefined> {
    try { return JSON.parse(await readFile(this.path(key), "utf8")) as RepositoryEnrollmentV1; }
    catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return undefined; throw error; }
  }
  async put(key: string, value: RepositoryEnrollmentV1): Promise<void> {
    await mkdir(this.root, { recursive: true, mode: 0o700 });
    const path = this.path(key); const temporary = `${path}.${randomUUID()}.tmp`;
...
export class RepositoryEnrollmentService {
  ...
  async plan(cwd: string): Promise<{ key: string; enrollment?: RepositoryEnrollmentV1; plan: RepositoryEnrollmentPlanV1 }> {
    ...
    const canonicalRepositoryPath = detected.mode === "existing_jj"
      ? await this.repositoryStorePath(repositoryPath)
      : undefined;
    const priorPrivateCommits = detected.mode === "existing_jj"
      ? (await this.optional(repositoryPath, ["config", "get", "git.private-commits"]))?.trim() || "none()"
  ...
  async enroll(plan: RepositoryEnrollmentPlanV1, authorization: RepositoryEnrollmentAuthorization): Promise<RepositoryEnrollmentReceiptV1> {
    if (authorization.planDigest !== plan.planDigest || enrollmentPlanDigest(withoutPlanDigest(plan)) !== plan.planDigest) {
    ...
    await this.store.put(provisionalKey, { version: 1, phase: "initializing", plan, userAuthorizationId: authorization.authorizationId, boundary: "authorized", startedAt, evidence: { authorizedAt: authorization.authorizedAt } });
    this.trigger("authorized");
    try {
      if (plan.initializationMode !== "existing_jj") {
        await this.run(plan.repositoryPath, ["git", "init", "--colocate", plan.repositoryPath], "write");
      }
      await this.boundary(provisionalKey, plan, authorization, startedAt, "repository_initialized", {});
      const repositoryStore = await this.repositoryStorePath(plan.repositoryPath);
      await this.run(plan.repositoryPath, ["config", "set", "--repo", PRIVATE_ALIAS_NAME, PRIVATE_EXPRESSION], "write");
      await this.boundary(provisionalKey, plan, authorization, startedAt, "alias_configured", { repositoryStore });
      await this.run(plan.repositoryPath, ["config", "set", "--repo", "git.private-commits", plan.nextPrivateCommits], "write");
      await this.boundary(provisionalKey, plan, authorization, startedAt, "private_commits_configured", { repositoryStore });
      await mkdir(plan.managedWorkspaceRoot, { recursive: true, mode: 0o700 });
      await this.boundary(provisionalKey, plan, authorization, startedAt, "workspace_root_created", { repositoryStore });
      const alias = (await this.run(plan.repositoryPath, ["config", "get", PRIVATE_ALIAS_NAME], "read")).trim();
      const privateCommits = (await this.run(plan.repositoryPath, ["config", "get", "git.private-commits"], "read")).trim();
      if (alias !== PRIVATE_EXPRESSION || !containsAlias(privateCommits)) throw new Error("Repository private-commit configuration verification failed.");
      await this.run(plan.repositoryPath, ["log", "--revision", PRIVATE_ALIAS, "--no-graph", "--template", 'change_id ++ "\\n"'], "read");
      ...
      const receipt: RepositoryEnrollmentReceiptV1 = { ...receiptBase, receiptDigest: enrollmentReceiptDigest(receiptBase) };
      ...
      await this.store.put(provisionalKey, {
        version: 1, phase: "attention_required", plan, lastSafeBoundary,
        reason: error instanceof Error ? error.message : String(error), evidence: current, stoppedAt: this.now(),
      });
      throw error;
    }
  }
  ...
  async verify(cwd: string): Promise<RepositoryEnrollmentReceiptV1> {
    ...
    const enrollment = await this.store.get(key);
    if (!enrollment || enrollment.phase !== "ready") throw new Error("Repository is not enrolled with Pi-Tai. Run /init-pi-tai.");
    if (enrollment.receipt.canonicalRepositoryPath !== repositoryStore) throw new Error("Repository enrollment identity changed.");
    const alias = (await this.optional(cwd, ["config", "get", PRIVATE_ALIAS_NAME]))?.trim();
    const privateCommits = (await this.optional(cwd, ["config", "get", "git.private-commits"]))?.trim();
    if (alias !== PRIVATE_EXPRESSION || !privateCommits || !containsAlias(privateCommits)) {
      const drifted: RepositoryEnrollmentV1 = { version: 1, phase: "repair_required", receipt: enrollment.receipt, reason: "Repo-local Pi-Tai private-commit configuration drifted.", observedAt: this.now() };
      await this.store.put(key, drifted);
      throw new Error("Repository enrollment configuration drifted. Re-run /init-pi-tai to review repairs.");
    }
    if (digest({ alias, privateCommits }) !== enrollment.receipt.configDigest) {
      const drifted: RepositoryEnrollmentV1 = { version: 1, phase: "repair_required", receipt: enrollment.receipt, reason: "Repo-local private-commit expression changed after enrollment.", observedAt: this.now() };
      await this.store.put(key, drifted);
      throw new Error("Repository enrollment configuration changed. Re-run /init-pi-tai to review repairs.");
    }
    await access(enrollment.receipt.managedWorkspaceRoot);
    return enrollment.receipt;
  }

  private async boundary(key: string, plan: RepositoryEnrollmentPlanV1, authorization: RepositoryEnrollmentAuthorization, startedAt: string, boundary: Extract<RepositoryEnrollmentV1, { phase: "initializing" }>["boundary"], evidence: unknown): Promise<void> {
    await this.store.put(key, { version: 1, phase: "initializing", plan, userAuthorizationId: authorization.authorizationId, boundary, startedAt, evidence });
    this.trigger(boundary);
  }
  private trigger(boundary: string): void { this.failpoint?.(boundary); }
  private async detect(start: string): Promise<{ root: string; mode: RepositoryInitializationMode }> {
    ...
    return gitRoot ? { root: gitRoot, mode: "colocate_git" } : { root: start, mode: "new_colocated" };
  }
  ...
  private optional(cwd: string, args: readonly string[]): Promise<string | undefined> {
    return this.executor.execute({ cwd: absolutePath(resolve(cwd)), args, access: "read" }).then((result) => result.kind === "success" ? result.stdout : undefined);
  }
...
async function nearestMarker(start: string, marker: string): Promise<string | undefined> {
  ...
  while (true) {
    try { await access(join(current, marker)); return current; } catch { /* continue */ }
    const parent = dirname(current); if (parent === current) return undefined; current = parent;
  }
}
function containsAlias(expression: string): boolean { return /\bpi_tai_private\s*\(\s*\)/.test(expression); }
function repositoryKey(path: string): string { return digest(resolve(path)); }
...
function withoutPlanDigest(plan: RepositoryEnrollmentPlanV1): Omit<RepositoryEnrollmentPlanV1, "planDigest"> { const { planDigest: _, ...rest } = plan; return rest; }
