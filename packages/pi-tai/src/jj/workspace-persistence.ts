...
export interface PersistedWorkspaceOperationV1 {
  readonly operationId: string;
  readonly kind: "allocate_workspace" | "workspace_checkpoint" | "rebase_workspace" | "normalize_change_range" | "prepare_workspace_report";
...
function validateAllocation(value: unknown): void { if (!object(value)) throw new Error("Workspace allocation is required."); for (const key of ["allocationId", "operationId", "rootSessionId", "ownerContextId", "sourceId"] as const) managedId(nonempty(value[key], key), key); for (const key of ["sourceWipChangeId", "baseChangeId"] as const) fullChangeId(nonempty(value[key], key)); digest(nonempty(value.sourcePatchHash, "sourcePatchHash")); workspaceName(nonempty(value.name, "name")); absolute(nonempty(value.path, "path")); nonempty(value.startedAt, "startedAt"); nonempty(value.beforeJjOperationId, "beforeJjOperationId"); }
function validateIdentity(value: unknown): asserts value is PersistedWorkspaceIdentityV1 { if (!object(value)) throw new Error("Workspace identity is required."); for (const key of ["workspaceId", "rootSessionId", "sourceId"] as const) managedId(nonempty(value[key], key), key); for (const key of ["sourceWipChangeId", "baseChangeId", "rootChangeId", "expectedHeadChangeId"] as const) fullChangeId(nonempty(value[key], key)); workspaceName(nonempty(value.name, "name")); absolute(nonempty(value.path, "path")); }
