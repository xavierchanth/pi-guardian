...
export type PersistedIsolatedWorkspaceV1 =
  ...
  | ({ readonly version: 1; readonly phase: "active"; readonly identity: PersistedWorkspaceIdentityV1; readonly writer: PersistedWorkspaceWriterV1; readonly targets: readonly PersistedSharedTargetV1[]; readonly claims: readonly PersistedFileSetClaimV1[]; readonly operations: readonly PersistedWorkspaceOperationV1[]; readonly reviewCycle?: number; readonly priorReviewIds?: readonly string[]; readonly repair?: { readonly phase: "prepared" | "running" | "attention_required"; readonly attempt: number; readonly priorReviewId: string; readonly findingIds: readonly string[]; readonly affectedPaths: readonly string[]; readonly implementationRole: "worker" | "implementation-lead" | "documenter"; readonly repairWorkOrderId: string; readonly contextId?: string; readonly executionCycleId?: string; readonly reason?: string }; readonly createdAt: string; readonly updatedAt: string })
  ...
  | ({ readonly version: 1; readonly phase: "conflict_reconciliation"; readonly implementationEventId: string; readonly taskId: string; readonly reviewId: string; readonly approval: unknown; readonly integrationReceipt: unknown; readonly conflictPaths: readonly string[]; readonly focusedReviewId?: string } & PersistedFrozenCustodyV1)
...
export function validateIsolatedWorkspace(input: unknown): PersistedIsolatedWorkspaceV1 {
  ...
  if (input.phase === "conflict_resolution") input.phase = "conflict_reconciliation";
  const phase = nonempty(input.phase, "phase");
  const phases = ["allocating", "active", "reported", "acknowledged", "reviewing", "changes_requested", "approved", "integrating", "conflict_reconciliation", "integrated", "verifying", "closed", "closed_no_changes", "cleanup_pending", "incident"]; 
...
function validateTargets(value: unknown, workingChangeId: string): void { if (!Array.isArray(value)) throw new Error("Active workspace targets must be an array."); const ids = new Set<string>(); for (const target of value) { if (!object(target)) throw new Error("Workspace target is invalid."); fullChangeId(nonempty(target.changeId, "target.changeId")); if (target.wipChangeId !== undefined) { fullChangeId(nonempty(target.wipChangeId, "target.wipChangeId")); target.baseChangeId ??= target.wipChangeId; target.workingChangeId ??= target.wipChangeId; delete target.wipChangeId; } if (ids.has(target.changeId)) throw new Error("Duplicate workspace target Change ID."); ids.add(target.changeId); if (target.baseChangeId !== workingChangeId || target.workingChangeId !== workingChangeId) throw new Error("Workspace target must bind the stable workspace head."); managedId(nonempty(target.ownerContextId, "target.ownerContextId"), "target owner"); nonempty(target.description, "target.description"); managedId(nonempty(target.insertOperationId, "target.insertOperationId"), "operation"); nonempty(target.createdAt, "target.createdAt"); } }
function validateClaims(value: unknown, targets: unknown): void { if (!Array.isArray(value) || !Array.isArray(targets)) throw new Error("Active workspace claims must be an array."); const targetIds = new Set(targets.filter(object).map((target) => target.changeId)); const ids = new Set<string>(); for (const claim of value) { if (!object(claim)) throw new Error("Workspace claim is invalid."); if (claim.wipChangeId !== undefined) { fullChangeId(nonempty(claim.wipChangeId, "claim.wipChangeId")); claim.baseChangeId ??= claim.wipChangeId; claim.workingChangeId ??= claim.wipChangeId; delete claim.wipChangeId; } fullChangeId(nonempty(claim.baseChangeId, "claim.baseChangeId")); fullChangeId(nonempty(claim.workingChangeId, "claim.workingChangeId")); const id = nonempty(claim.claimId, "claim.claimId"); managedId(id, "claim"); if (ids.has(id)) throw new Error("Duplicate workspace claim ID."); ids.add(id); if (!targetIds.has(claim.targetChangeId)) throw new Error("Workspace claim target is not assigned in this workspace."); if (!Array.isArray(claim.paths) || !claim.paths.length) throw new Error("Workspace claim requires paths."); const phase = nonempty(claim.phase, "claim.phase"); if (!["queued", "active", "checkpointing", "released", "interrupted", "breached"].includes(phase)) throw new Error("Invalid workspace claim phase."); } }
function validateRepair(value: unknown): void { if (!object(value) || !["prepared", "running", "attention_required"].includes(String(value.phase))) throw new Error("Invalid review-repair state."); if (!Number.isSafeInteger(value.attempt) || value.attempt < 1) throw new Error("Repair attempt must be positive."); managedId(nonempty(value.priorReviewId, "repair review"), "review"); managedId(nonempty(value.repairWorkOrderId, "repair work order"), "task"); if (!["worker", "implementation-lead", "documenter"].includes(String(value.implementationRole))) throw new Error("Invalid repair implementation role."); if (!Array.isArray(value.findingIds) || !value.findingIds.length || !Array.isArray(value.affectedPaths) || !value.affectedPaths.length) throw new Error("Repair requires findings and affected paths."); if (value.phase === "running") { managedId(nonempty(value.contextId, "repair context"), "context"); managedId(nonempty(value.executionCycleId, "repair cycle"), "cycle"); } if (value.phase === "attention_required") nonempty(value.reason, "repair attention reason"); }
