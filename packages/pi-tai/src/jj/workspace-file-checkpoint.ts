...
export class WorkspaceFileCheckpointer {
  ...
  async checkpoint(id: WorkspaceId, claim: CheckpointableFileSetClaim): Promise<JjOperationResult<CheckpointChangeReceipt>> {
    ...
    return this.repository.mutate(id, async (identity) => {
      ...
      if (!target || target.wipChangeId !== identity.expectedHeadChangeId) return { kind: "blocked", blocker: { kind: "foreign_work", reason: "Workspace claim no longer matches its assigned target and stable WIP." } };
      ...
      const operation: PersistedWorkspaceOperationV1 = { operationId, kind: "workspace_file_checkpoint", idempotencyKey: `workspace-file-checkpoint:${claim.claimId}`, startedAt: at, beforeJjOperationId, intent: { claimId: claim.claimId, wipChangeId: wipId, targetChangeId: targetId, ownerContextId: refreshed.record.ownerContextId, paths: refreshed.record.paths, mutatedPaths: refreshed.record.mutatedPaths, beforeUnownedHash }, outcome: { phase: "started", boundary: "prepared" } };
      ...
      try {
        ...
        const [afterWip, afterTarget, afterCurrent] = await Promise.all([this.repository.resolveTracked(id, wipId), this.repository.resolveTracked(id, targetId), this.repository.currentChangeId(id)]);
        ...
        if (afterCurrent !== wipId || afterUnownedHash !== beforeUnownedHash || remainingOwned.length) throw new Error("Workspace file checkpoint did not preserve stable WIP ownership boundaries.");
        ...
        const receipt: CheckpointChangeReceipt = { checkpointedChangeId: targetId, wipChangeId: wipId, claimId: claim.claimId, changedPaths, parentChangeIds: afterTarget.parentChangeIds, unownedWipPatchHash: afterUnownedHash, conflicted: afterWip.conflicted || afterTarget.conflicted, operationId: jjOperationId(operationId) };
