...
export type PersistedJjOperationV1 = PersistedOperationBaseV1 &
  (
    ...
    | {
        ...
        readonly afterJjOperationId: string;
...
export class HostSharedSourceStore implements SharedSourceStore {
  ...
  constructor(state: HostConcurrencyState) {
    this.state = state;
  }
  async create(record: PersistedSharedSourceV1): Promise<void> {
    const value = validateSharedSource(record);
    await this.state.mutateSegment<PersistedSharedSourceV1>(
      "sharedSources",
      "source.created",
      { sourceId: value.sourceId },
      (sources) => {
        if (sources.some((source) => source.sourceId === value.sourceId))
          throw new Error(`Shared source already exists: ${value.sourceId}`);
        return [...sources, value];
      },
    );
  }
  async get(sourceId: string): Promise<PersistedSharedSourceV1 | undefined> {
    return (await this.list()).find((source) => source.sourceId === sourceId);
  }
  async list(): Promise<PersistedSharedSourceV1[]> {
    return (await this.state.readSegment<PersistedSharedSourceV1>("sharedSources")).map(
    ...
    await this.state.mutateSegment<PersistedSharedSourceV1>(
      ...
      (sources) =>
        sources.map((source) => {
          ...
          const next = validateSharedSource(reducer(structuredClone(source)));
          if (next.sourceId !== sourceId) throw new Error("Shared source update changed identity.");
          output = next;
          return next;
        }),
    );
    if (!output) throw new Error(`Unknown shared source: ${sourceId}`);
    return output;
  }
  interruptLiveClaims(
    sourceId: string,
...
export class FileSharedSourceStore implements SharedSourceStore {
  ...
  async create(record: PersistedSharedSourceV1): Promise<void> {
    ...
    await writeFile(this.path(value.sourceId), serialize(value), {
      encoding: "utf8",
      mode: 0o600,
      flag: "wx",
    });
  ...
  update(
    ...
    let release!: () => void;
    const current = new Promise<void>((resolveUpdate) => {
      release = resolveUpdate;
    });
    ...
    return prior
      .then(async () => {
        ...
        if (next.sourceId !== sourceId)
          throw new Error("Shared source update cannot change identity.");
...
export function validateSharedSource(input: unknown): PersistedSharedSourceV1 {
  if (!record(input) || input.version !== 1)
    throw new Error("Shared source record must use version 1.");
  ...
  if (input.wip !== undefined) {
    validateWip(input.wip);
    delete input.wip;
  }
  const targets = new Set<string>();
  for (const target of input.targets) {
    validateTarget(target);
    if (targets.has(target.changeId))
      throw new Error(`Duplicate shared target Change ID: ${target.changeId}`);
  ...
  for (const operation of input.operations) {
    ...
    if (operations.has(operation.operationId))
      throw new Error(`Duplicate managed JJ operation ID: ${operation.operationId}`);
  ...
  input.operations = input.operations.filter(
    (operation: Record<string, unknown>) => operation.kind !== retiredLegacyOperationKind,
  );
...
function migrateWorkingIdentity(value: Record<string, any>, label: string): void {
  if (value.wipChangeId !== undefined) {
    ...
    fullChangeId(legacy);
    if (value.workingChangeId !== undefined && value.workingChangeId !== legacy)
      throw new Error(`${label} legacy and working Change IDs disagree.`);
...
function validateClaim(value: unknown): asserts value is PersistedFileSetClaimV1 {
  ...
  if (
    !["queued", "active", "checkpointing", "released", "interrupted", "breached"].includes(phase)
  ) {
  ...
  if (!Array.isArray(value.paths) || value.paths.length === 0)
    throw new Error("Claim paths must be a nonempty array.");
  let prior = "";
  ...
  if (phase === "active")
    forbid(
      value,
      [...checkpointFields, ...releasedFields, ...interruptedFields, ...breachedFields],
      "active claim",
    );
  if (phase === "checkpointing")
    forbid(
      value,
      [...releasedFields, ...interruptedFields, ...breachedFields],
      "checkpointing claim",
    );
  if (phase === "released")
    forbid(
      value,
      [...activeFields, ...checkpointFields, ...interruptedFields, ...breachedFields],
      "released claim",
    );
  if (phase === "interrupted")
    forbid(
      value,
      [...activeFields, ...checkpointFields, ...releasedFields, ...breachedFields],
      "interrupted claim",
    );
  if (phase === "breached")
    forbid(
  ...
  if (phase === "active" || phase === "checkpointing") {
    ...
    if (!Array.isArray(value.mutatedPaths))
      throw new Error(`${phase} claim requires mutatedPaths.`);
    for (const path of value.mutatedPaths) repositoryPath(nonempty(path, "claim mutated path"));
  }
  if (phase === "checkpointing")
    validateManagedId(nonempty(value.operationId, "claim.operationId"), "operation");
  if (phase === "released") {
    nonempty(value.releasedAt, "claim.releasedAt");
    if (value.checkpointOperationId !== undefined)
      validateManagedId(
        nonempty(value.checkpointOperationId, "claim.checkpointOperationId"),
        "operation",
      );
  }
  if (phase === "interrupted") {
    if (!["queued", "active", "checkpointing"].includes(String(value.priorPhase)))
      throw new Error("Interrupted claim prior phase is invalid.");
    nonempty(value.reason, "claim.reason");
    nonempty(value.interruptedAt, "claim.interruptedAt");
    if (value.recovery !== undefined) {
      if (
        !record(value.recovery) ||
        !Array.isArray(value.recovery.fingerprints) ||
        !Array.isArray(value.recovery.mutatedPaths)
      ) {
      ...
      if (
        !/^[a-f0-9]{64}$/.test(
          nonempty(value.recovery.baselinePatchHash, "claim.recovery.baselinePatchHash"),
        )
      ) {
        throw new Error("Interrupted claim recovery baseline patch hash must be SHA-256.");
      }
      for (const path of value.recovery.mutatedPaths)
        repositoryPath(nonempty(path, "claim recovery mutated path"));
      if (value.recovery.operationId !== undefined)
        validateManagedId(
          nonempty(value.recovery.operationId, "claim.recovery.operationId"),
          "operation",
        );
    }
  }
...
function validateFingerprint(value: unknown): void {
  ...
  if (!/^[a-f0-9]{64}$/.test(nonempty(value.digest, "fingerprint.digest")))
    throw new Error("Path fingerprint digest must be SHA-256.");
}
...
function validateOperation(value: unknown): asserts value is PersistedJjOperationV1 {
  ...
  if (!["started", "completed", "blocked", "unknown"].includes(phase))
    throw new Error(`Invalid managed operation phase: ${phase}`);
  validateManagedId(nonempty(value.operationId, "operation.operationId"), "operation");
  const retiredLegacyOperationKind = ["ensure", "wip"].join("_");
  if (
    ![retiredLegacyOperationKind, "insert_change", "checkpoint_change"].includes(
      nonempty(value.kind, "operation.kind"),
    )
  ) {
  ...
  if (phase === "started")
    forbid(value, [...completedFields, ...blockedFields, ...unknownFields], "started operation");
  if (phase === "completed")
    forbid(value, [...blockedFields, ...unknownFields], "completed operation");
  if (phase === "blocked")
    forbid(value, [...completedFields, ...unknownFields], "blocked operation");
  if (phase === "unknown")
    forbid(value, [...completedFields, ...blockedFields], "unknown operation");
  if (phase === "completed") {
...
function forbid(value: Record<string, unknown>, fields: readonly string[], label: string): void {
  ...
  if (present.length)
    throw new Error(`${label} contains fields from another phase: ${present.join(", ")}`);
}
