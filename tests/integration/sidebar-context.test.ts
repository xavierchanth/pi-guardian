...
test("persisted /btw entries stay outside context after reload and branch", async () => {
  ...
  try {
    ...
    const persisted: BtwEntry = { state: "success", question: "SECRET_Q", answer: "SECRET_A", model: "test/model", timestamp: new Date(0).toISOString(), truncation: { input: false, output: false } };
    ...
    const branched: BtwEntry = { state: "success", question: "BRANCH_Q", answer: "BRANCH_A", model: "test/model", timestamp: new Date(0).toISOString(), truncation: { input: false, output: false } };
