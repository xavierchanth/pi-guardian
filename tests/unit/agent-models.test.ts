...
describe("model resolution", () => {
  ...
  it("loads the supported model aliases from the packaged catalog", () => {
    ...
    assert.deepEqual(MODEL_ALIAS_NAMES, [
      "fable",
      "glm",
      "kimi",
      "luna",
  ...
  it("rejects explicit provider/model ids on an incompatible harness", () => {
    ...
    assert.equal(
      resolveModel({ model: "openrouter/anthropic/claude-sonnet-5", backend: "pi" }).ok,
      false,
    );
  ...
  it("rejects a bare name that is neither alias nor provider/model", () => {
    ...
    assert.match(
      ...
      /neither a known alias .*fable.*nor a provider\/model/,
    );
  });
});
