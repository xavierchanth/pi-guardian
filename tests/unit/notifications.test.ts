...
test("notification settings and headless modes suppress output", () => {
...
test("terminal notification sequences support Kitty and OSC 777 safely", () => {
  assert.equal(
    terminalNotificationSequence({ KITTY_WINDOW_ID: "1" }, "Pi", "Ready"),
    "\x1b]99;i=pi-tai:d=0;Pi\x1b\\\x1b]99;i=pi-tai:p=body;Ready\x1b\\",
  );
  const generic = terminalNotificationSequence({}, "Pi;bad", "Done\x07now");
  assert.equal(generic, "\x1b]777;notify;Pi bad;Done now\x07");
});
