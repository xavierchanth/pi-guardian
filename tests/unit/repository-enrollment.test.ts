...
test("repository enrollment preserves user workspace and installs private pi-tai policy", async () => {
  ...
  try {
    ...
    try {
      const service = new RepositoryEnrollmentService({
        store: new FileRepositoryEnrollmentStore(state),
        executor: fixture.executor,
      });
      const planned = await service.plan(fixture.repoPath);
      assert.equal(planned.plan.initializationMode, "existing_jj");
      const receipt = await service.enroll(planned.plan, {
        authorizationId: "user-consent-1",
      ...
      assert.equal(
        (
          await fixture.run(fixture.repoPath, [
            "config",
            "get",
            'revset-aliases."pi_tai_private()"',
          ])
        ).trim(),
        'description(glob:"pi-tai:*")',
      );
      assert.match(
        (await fixture.run(fixture.repoPath, ["config", "get", "git.private-commits"])).trim(),
        /pi_tai_private/,
      );
  ...
  try {
    ...
    try {
      const service = new RepositoryEnrollmentService({
        ...
        executor: fixture.executor,
        failpoint: (boundary) => {
          if (boundary === "alias_configured") throw new Error("stop");
        },
      });
      const planned = await service.plan(fixture.repoPath);
      await assert.rejects(
        service.enroll(planned.plan, {
          authorizationId: "user-consent-2",
          planDigest: planned.plan.planDigest,
          authorizedAt: "now",
        }),
        /stop/,
      ...
      const records = await Promise.all(
        files.map((file) => readFile(join(state, file), "utf8").then(JSON.parse)),
      );
      assert.ok(
        records.some(
          (record) =>
            record.phase === "attention_required" && record.lastSafeBoundary === "alias_configured",
        ),
      );
    } finally {
      await rm(state, { recursive: true, force: true });
