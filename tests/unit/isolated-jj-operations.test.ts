...
test("tracked isolated workspace allocates, checkpoints with a fresh lease, and freezes", async () => {
  ...
  try {
    ...
    const ensured = await runtime.shared.operations.ensureWip(source); assert.equal(ensured.kind, "completed");
...
test("interrupted rebase reconstructs preserved range identity", async () => {
  ...
  try {
    ...
    const source = await runtime.shared.openSource(fixture.repoPath); assert.equal((await runtime.shared.operations.ensureWip(source)).kind, "completed"); const created = await runtime.operations.createWorkspace(source, { name: workspaceName("rebase-recover"), ownerContextId: "child-r", rootSessionId: "root-1" }); assert.equal(created.kind, "completed"); if (created.kind !== "completed") return;
