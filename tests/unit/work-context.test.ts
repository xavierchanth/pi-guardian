...
test("builds the approved two-line status presentation", () => {
  ...
  assert.equal(collapsedWorkContextText(snapshot), "✓ Plan 1/3 · Now: Integrate Guardian");
...
test("presents empty and completed plans clearly", () => {
  ...
  assert.deepEqual(workContextStatusLines(empty), ["Goal: Answer", "Plan: No active steps"]);
...
test("reconstructs the latest valid snapshot on the active branch", () => {
  const first = workContextDetails(
    validateWorkContextUpdate({
      goal: "First",
      plan: [{ content: "one", status: "in_progress" }],
    }),
  );
  const second = workContextDetails(
    validateWorkContextUpdate({
      goal: "Second",
      plan: [],
    }),
