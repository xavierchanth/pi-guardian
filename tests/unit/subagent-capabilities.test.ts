...
describe("capability subagent catalog", () => {
  ...
  it("rejects malformed catalog objects", () => {
    ...
    assert.throws(() => parseCapabilityCatalog({ ...valid, capabilities: [] }), /must define exactly/);
    assert.throws(() => parseCapabilityCatalog({ ...valid, capabilities: [{ ...capability, effort: "extreme" }] }), /effort must be one of/);
    assert.throws(() => parseCapabilityCatalog({ ...valid, capabilities: [{ ...capability, model: "unknown" }] }), /model must be a known model alias/);
    assert.throws(() => parseCapabilityCatalog({ ...valid, capabilities: [{ ...capability, backend: "browser" }] }), /backend must be one of/);
    assert.throws(() => parseCapabilityCatalog({ ...valid, capabilities: [capability, capability] }), /Duplicate capability/);
  });
