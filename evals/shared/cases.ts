...
export type SubagentSetup = {
  kind: "subagent-harness";
  parentRole: SubagentParentRole;
  childRole: "implementation-lead" | "worker";
...
export type SubagentInteraction =
  ...
  | {
      kind: "recursive-implementation-lead-workspace";
      requesterRole: "implementation-lead" | "worker";
    }
  | { kind: "inspect-child-record"; outcome: "failed" | "cancelled" };
export type TranscriptEvent =
  | "parent-steer"
  | "visible-bounded-status"
  | "subsequent-work"
  | "tool-activity"
  | "terminal-report";
export type SubagentAssertion =
  | {
      ...
      invariant:
        ...
        | "collect-every-child"
        | "question-resumes-child"
        | "delivery-semantics"
        | "single-terminal-report"
        | "block-unresolved-parent"
        | "deny-recursive-implementation-lead-workspace"
        | "terminal-record-inspectable";
    }
  | { kind: "transcript-sequence"; events: TranscriptEvent[] }
  | { kind: "report-count"; phase: "interim" | "terminal"; count: number };
export type SubagentCase = {
  version: 1;
  suite: "subagents";
  id: string;
  title: string;
  ...
  assertions: SubagentAssertion[];
};

function fail(source: string, path: string, message: string): never {
  throw new Error(`${source}:${path}: ${message}`);
}
function mapping(value: unknown, source: string, path: string): Record<string, unknown> {
...
function exact(
  ...
  for (const key of Object.keys(value))
    if (!allowed.includes(key)) fail(source, `${path}.${key}`, "unknown field");
}
function str(value: unknown, source: string, path: string): string {
...
function integer(value: unknown, source: string, path: string): number {
  if (!Number.isInteger(value) || (value as number) < 0)
    fail(source, path, "expected non-negative integer");
...
function subagentSetup(raw: unknown, source: string): SubagentSetup {
  ...
  const parentRole = oneOf(
    value.parentRole,
    ["root", "implementation-lead"] as const,
    source,
    "setup.parentRole",
  );
  const childRole = oneOf(
    value.childRole,
    ["implementation-lead", "worker"] as const,
    source,
    "setup.childRole",
  );
  if (
    (parentRole === "root" && childRole !== "implementation-lead") ||
    (parentRole === "implementation-lead" && childRole !== "worker")
  ) {
    fail(
      source,
      "setup.childRole",
      `${parentRole} cannot directly launch ${childRole} in the packaged hierarchy`,
    );
  }
  return {
    kind: "subagent-harness",
...
function subagentInteraction(raw: unknown, source: string): SubagentInteraction {
  ...
  const kind = oneOf(
    value.kind,
    [
      "launch-without-collection",
      "wait-any-collection",
      "question-response-resume",
      "rpc-delivery",
      "interim-status-resume",
      "terminal-report",
      "parent-completion",
      "recursive-implementation-lead-workspace",
  ...
  switch (kind) {
    ...
    case "question-response-resume":
      ...
      return {
        kind,
        question: str(value.question, source, `${path}.question`),
        response: str(value.response, source, `${path}.response`),
      };
    case "rpc-delivery":
      exact(value, ["kind", "delivery", "message"], source, path);
      return {
        kind,
        delivery: oneOf(value.delivery, ["steer", "followUp"] as const, source, `${path}.delivery`),
        message: str(value.message, source, `${path}.message`),
      };
    case "interim-status-resume":
      exact(value, ["kind", "role", "parentMessage"], source, path);
      return {
        kind,
        role: oneOf(value.role, ["implementation-lead", "worker"] as const, source, `${path}.role`),
        parentMessage: str(value.parentMessage, source, `${path}.parentMessage`),
      };
    case "terminal-report":
      exact(value, ["kind", "attemptedReports"], source, path);
      return {
        kind,
        attemptedReports: integer(value.attemptedReports, source, `${path}.attemptedReports`),
      };
    case "parent-completion":
      exact(value, ["kind", "unresolvedDescendants"], source, path);
      return {
        kind,
        unresolvedDescendants: integer(
          value.unresolvedDescendants,
          source,
          `${path}.unresolvedDescendants`,
        ),
      };
    ...
    case "inspect-child-record":
      ...
      return {
        kind,
        outcome: oneOf(value.outcome, ["failed", "cancelled"] as const, source, `${path}.outcome`),
      };
  }
}
...
function subagentAssertion(raw: unknown, source: string, index: number): SubagentAssertion {
  ...
  const kind = oneOf(
    value.kind,
    ["protocol-invariant", "transcript-sequence", "report-count"] as const,
    source,
    `${path}.kind`,
  );
  ...
  if (kind === "report-count") {
    ...
    return {
      kind,
      phase: oneOf(value.phase, ["interim", "terminal"] as const, source, `${path}.phase`),
      count: integer(value.count, source, `${path}.count`),
    };
  }
  exact(value, ["kind", "events"], source, path);
  if (!Array.isArray(value.events) || value.events.length === 0)
    fail(source, `${path}.events`, "expected non-empty sequence");
  return {
    kind,
...
export function validateCase(raw: unknown, source = "<input>"): SubagentCase {
  ...
  exact(
    value,
    ["version", "suite", "id", "title", "execution", "setup", "interaction", "assertions"],
    source,
    "$",
  );
  if (value.version !== 1) fail(source, "version", "expected 1");
  if (value.suite !== "subagents") fail(source, "suite", "expected subagents");
  if (value.execution !== "specification-only")
    fail(source, "execution", "subagent cases must be specification-only");
  ...
  if (!Array.isArray(value.assertions) || value.assertions.length === 0)
    fail(source, "assertions", "expected non-empty sequence");
  return {
    version: 1,
    suite: "subagents",
    id,
    ...
    assertions: value.assertions.map((assertion, index) =>
      subagentAssertion(assertion, source, index),
    ),
  };
}
...
export async function loadCases(
  directory: string,
  expectedSuite: "subagents" = "subagents",
): Promise<SubagentCase[]> {
