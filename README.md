...
Pi-Tai includes a standalone autonomy-first action guardian. Every agent-generated `bash` and `web_fetch` call is reviewed by `openai-codex/codex-auto-review` using Pi's Codex OAuth provider. The reviewer classifies risk, authorization basis (`none`, `task`, or `explicit`), and impact scope; deterministic code derives the outcome. Reasonable methods within the user's task—including routine inspection, edits, tests, builds, dependency work, and research—proceed without method-level permission. High-risk work requires task or explicit authority plus bounded effects, while critical work never executes automatically. Guardian never asks for approval: every non-allow result becomes a failed tool result and the agent continues with other authorized work. Invalid, timed-out, cancelled, and failed reviews fail closed. Non-allow review records are retained locally under `~/.pi/agent/pi-tai/guardian-reviews/` for evaluation.
