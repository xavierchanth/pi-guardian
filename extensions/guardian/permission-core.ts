...
type ShellToken = string | { op: string } | { comment: true };
...
function tokenizeShell(command: string): ShellToken[] {
	...
	while (i < command.length) {
		...
		if (ch === '"') {
			...
			if (i < command.length) str += command[i++];
			tokens.push(str);
			continue;
		}
		...
		while (i < command.length) {
			...
			if (/\s/.test(c)) break;
			if (c === '#' || c === '"' || c === "'") break;
			const isOp = OPS.some((op) => command.substring(i, i + op.length) === op);
			if (isOp) break;
...
const LEGACY_LEVEL_MAP: Record<string, PermissionLevel> = {
	minimal: "read",
	low: "edit",
	medium: "auto",
	high: "auto",
...
function migrateLevel(level: string): PermissionLevel | null {
	const normalized = level.toLowerCase();
	if (LEVELS.includes(normalized as PermissionLevel)) {
...
function validateConfig(config: unknown): PermissionConfig {
	...
	if (raw.overrides && typeof raw.overrides === "object") {
		...
		const levels = ["read", "edit", "auto", "dangerous"] as const;
...
function checkOverrides(command: string, overrides: PermissionConfig["overrides"]): Classification | null {
	...
	if (overrides.dangerous && matchesAnyPattern(trimmed, overrides.dangerous)) {
		return { level: "auto", dangerous: true, needsReview: true };
	}

	// Check levels in order of specificity (high to low)
	if (overrides.auto && matchesAnyPattern(trimmed, overrides.auto)) {
		return { level: "auto", dangerous: false, needsReview: false };
	}

	if (overrides.edit && matchesAnyPattern(trimmed, overrides.edit)) {
		return { level: "edit", dangerous: false, needsReview: false };
	}

	if (overrides.read && matchesAnyPattern(trimmed, overrides.read)) {
		return { level: "read", dangerous: false, needsReview: false };
	}
...
export function loadGlobalPermission(): PermissionLevel | null {
	...
	if (level) {
		const migrated = migrateLevel(level);
...
export function loadAutoReviewModels(): string[] {
	const settings = loadSettings();
	const models = settings.autoReviewModels;
	if (Array.isArray(models)) {
		return models.filter((m): m is string => typeof m === "string" && m.includes("/"));
	}
	return [];
}
...
const CONDITIONAL_WRITE_COMMANDS: Record<string, (tokens: string[]) => boolean> = {
	...
	xargs: (tokens) => {
		...
		// Safe if running a read-only command from READ_COMMANDS
		...
		if (READ_COMMANDS.has(xargsCmd)) return false;

		// Unknown or non-read command - not safe
		return true;
	},
	tee: (tokens) => {
...
const READ_PACKAGE_SUBCOMMANDS: Record<string, Set<string>> = {
...
function isReadLevel(tokens: string[]): boolean {
	...
	if (conditionalCheck) {
		// If the command would write/execute, it's not read level
	...
	if (READ_COMMANDS.has(cmd)) return true;
	...
	// Git read operations
	if (cmd === "git" && subCmd && READ_GIT_SUBCOMMANDS.has(subCmd)) {
		// Some git commands are only read-only without additional args
		// e.g., "git branch" lists branches (read), "git branch new" creates (auto)
		// e.g., "git tag" lists tags (read), "git tag v1.0" creates (auto)
	...
	if (READ_PACKAGE_SUBCOMMANDS[cmd]?.has(subCmd)) {
		return true;
	}
...
// AUTO level - build/install/test operations only (NOT running code)
const AUTO_PACKAGE_PATTERNS: Array<[string, RegExp]> = [
	...
	// npx/bunx/pnpx run arbitrary packages - needs review (not included here)
	...
	["uv", /^(pip|sync|lock)$/],
	// python/python3 run arbitrary code - needs review (not included here)
	...
	["rustfmt", /./],
	// rustc compiles but doesn't run - auto
	["rustc", /./],
	...
	// rake/rails can run arbitrary code - needs review (not included here)
	["rspec", /./], // test runner

	// PHP - install only
	["composer", /^(install|require|remove|update|dump-autoload)$/],
	// php runs code - needs review (not included here)
	["phpunit", /./], // test runner
	...
	// gradlew can run arbitrary tasks - needs review (not included here)
	...
	// Linters/formatters - static analysis only (AUTO)
	["eslint", /./],
	["prettier", /./],
	["black", /./],
	["flake8", /./],
...
const AUTO_GIT_SUBCOMMANDS = new Set([
	"add",
	"commit",
	"pull",
	"checkout",
...
function isAutoLevel(tokens: string[]): boolean {
	if (tokens.length === 0) return false;
	...
	if (cmd === "git") {
		if (subCmd === "push") return false; // push needs review
		if (subCmd === "reset" && tokens.includes("--hard")) return false; // hard reset needs review
		if (AUTO_GIT_SUBCOMMANDS.has(subCmd)) return true;
	}
	...
	for (const [pattern, subPattern] of AUTO_PACKAGE_PATTERNS) {
		if (cmd === pattern) {
...
// High-risk level - commands that need auto-review even in auto mode
function isHighRiskLevel(tokens: string[]): boolean {
	if (tokens.length === 0) return false;
...
function classifySegment(tokens: string[]): Classification {
	...
	// Shell execution commands that can run arbitrary code - require review
	if (SHELL_EXECUTION_COMMANDS.has(cmd)) {
		return { level: "auto", dangerous: false, needsReview: true };
	}
	...
	if (isReadLevel(tokens)) {
		return { level: "read", dangerous: false, needsReview: false };
	}

	if (isAutoLevel(tokens)) {
		return { level: "auto", dangerous: false, needsReview: false };
	}

	if (isHighRiskLevel(tokens)) {
		return { level: "auto", dangerous: false, needsReview: true };
	}

	// Default: require auto-review for unknown commands
	return { level: "auto", dangerous: false, needsReview: true };
}
...
export function classifyCommand(command: string, config?: PermissionConfig): Classification {
	...
	// require auto-review as we cannot reliably classify the embedded commands
	...
	let maxLevel: PermissionLevel = "read";
	let dangerous = false;
	let needsReview = false;

	// If command writes to files via redirection (>, >>), require at least EDIT
	...
	return { level: maxLevel, dangerous, needsReview };
}
